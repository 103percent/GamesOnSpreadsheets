...
df = pd.read_excel("gamename_quit_rates.xlsx")
...
highlight_indexes = [0,1,2,8,14]

# Line plot
sns.lineplot(data=df, x='chapter_name', y='quit_rate_pc', marker='o', linewidth=2.5, color='#007acc')
ax.yaxis.set_major_formatter(PercentFormatter(xmax=1))
ax.set_title("Nina Aquila: Legal Eagle, Chapter IV: Sacred Feathers", fontsize=18, weight='bold')
...
ab = AnnotationBbox(imagebox, (1, 1), xycoords='axes fraction', frameon=False,
                    box_alignment=(1.5, 1.1))  # Push it just outside the top right
ax.add_artist(ab)
